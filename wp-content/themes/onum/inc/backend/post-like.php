...
function get_like_count( $like_count ) {
	$like_text = __( 'Like', 'onum' );
	$likes_text = __( 'Likes', 'onum' );
	if ( is_numeric( $like_count ) && $like_count > 0 ) { 
		$number = '<span class="sl-count">' . sl_format_count( $like_count ) . '</span><span class="sl-text">' . $likes_text . '</span>';
	} else {
		$number = '<span class="sl-text">' . $like_text . '</span>';
	}
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'onum' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'onum' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
