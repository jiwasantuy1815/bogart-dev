    ...
    <div class="grid-x grid-margin-x grid-margin-y">
        ...
        foreach ($choices as $key => $_value)
        {
            ...
            <div class="cell medium-<?php echo esc_attr($this->data->get('cols', 12)); ?> choice-item">
                <input type="checkbox"<?php echo wp_kses_data($this->data->get('required_attribute', '')); ?> class="fpf-control-input-item" id="fpf-control-input-item_<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($this->data->get('name')); ?>[]" value="<?php echo esc_attr($key); ?>"<?php echo is_array($this->data->get('value')) && in_array($key, $this->data->get('value')) ? ' checked="checked"' : ''; ?> />
