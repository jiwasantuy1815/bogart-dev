...
class TagsProvider implements GetSelectedItems, GetSearchItems, GetItems
{
	/**
	 ...
	 * @var  String
	 */
	const join_clause = " AND iclt.element_type LIKE '%_tag'";
	...
	public function getItems($offset = 0, $limit = SearchDropdownHelper::SELECTION_ITEMS)
	{
		...
		if ($wpmlData = \FPFramework\Helpers\WPHelper::getWPMLQueryData('t.term_id', self::join_clause))
		{
			...
			$join = $wpmlData['join'];
			$where = "iclt.element_type = 'tax_post_tag' AND";
		}
    ...
    public function getSearchItems($name, $no_ids = null)
    {
		...
		$sql = "SELECT DISTINCT
				...
				WHERE
					t.name LIKE %s AND
					tt.taxonomy = 'post_tag'" . $where_end;
