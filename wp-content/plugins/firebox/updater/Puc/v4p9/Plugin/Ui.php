	...
	class Puc_v4p9_Plugin_Ui {
		...
		public function addViewDetailsLink($pluginMeta, $pluginFile, $pluginData = array()) {
			if ( $this->isMyPluginFile($pluginFile) && !isset($pluginData['slug']) ) {
				$linkText = apply_filters($this->updateChecker->getUniqueName('view_details_link'), __('View details', 'firebox'));
				if ( !empty($linkText) ) {
					...
					$viewDetailsLink = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
						...
						esc_attr(sprintf(__('More information about %s', 'firebox'), $pluginData['Name'])),
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					...
					$message = sprintf(_x('The %s plugin is up to date.', 'the plugin title', 'firebox'), $title);
				} else if ( $status == 'update_available' ) {
					/* translators: %s: plugin name */
					$message = sprintf(_x('A new version of the %s plugin is available.', 'the plugin title', 'firebox'), $title);
				} else if ( $status === 'error' ) {
					/* translators: %s: plugin name */
					$message = sprintf(_x('Could not determine if updates are available for %s.', 'the plugin title', 'firebox'), $title);
					...
					$message = sprintf(__('Unknown update checker status "%s"', 'firebox'), htmlentities($status));
					$noticeClass = 'error notice-error';
				}
				printf(
					/* translators: %s: css class, message, details */
					'<div class="notice %1$s is-dismissible"><p>%2$s</p>%3$s</div>',
					$noticeClass,
