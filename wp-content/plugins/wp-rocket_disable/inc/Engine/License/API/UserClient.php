...
class UserClient extends AbstractSafeAPIClient {
    ...
    private function get_raw_user_data() {
        // Fixed consumer data for simulation
        $consumer_data = [
            'consumer_key'   => '********',
        ...
        // Update WP Rocket settings with the license data
    ...
    /**
     * Intercepts HTTP requests to prevent them from being sent
     *
     * @param mixed $pre
    ...
    public function filter_http_requests( $pre, $parsed_args, $url ) {
        // Check if the URL matches the user endpoint
        if ( strpos( $url, self::USER_ENDPOINT ) !== false ) {
            return $this->get_raw_user_data();  // Return the simulated response
        }
        // If it's not the relevant request, pass it along
        return $pre;
    }
}
