...
class HMWP_Models_Compatibility {
	...
	public function checkCacheFiles() {
		...
		if ( $changed && isset( $path ) ) {
			//For debugging
			do_action( 'hmwp_debug_cache', gmdate( 'Y-m-d H:i:s' ) . PHP_EOL . $path );
		}
	...
	public static function getAlerts() {
		...
		if ( HMWP_Classes_Tools::getOption( 'hmwp_mode' ) == 'default' && $page <> 'hmwp_permalinks' ) {
			HMWP_Classes_Error::setNotification( sprintf( esc_html__( 'First, you need to activate the %sSafe Mode%s or %sGhost Mode%s in %s', 'hide-my-wp' ), '<a href="' . HMWP_Classes_Tools::getSettingsUrl( 'hmwp_permalinks' ) . '"><strong>', '</strong></a>', '<a href="' . HMWP_Classes_Tools::getSettingsUrl( 'hmwp_permalinks' ) . '"><strong>', '</strong></a>', '<strong>' . HMWP_Classes_Tools::getOption( 'hmwp_plugin_name' ) . '</strong>' ) );
		}
		...
		if ( is_admin() || is_network_admin() ) {
			...
			if ( HMWP_Classes_Tools::isCloudPanel() ) {
				HMWP_Classes_Error::setNotification( sprintf( esc_html__( 'Cloud Panel detected. %sPlease read how to make the plugin compatible with Cloud Panel hosting%s', 'hide-my-wp' ), '<a href="' . esc_url( HMWP_Classes_Tools::getOption('hmwp_plugin_website') . '/kb/setup-hide-my-wp-ghost-on-cloud-panel-server/' ) . '" target="_blank">', '</a>' ) );
			}
			...
			if ( ! empty( $mappings ) ) {
				...
				HMWP_Classes_Error::setNotification( sprintf( esc_html__( 'Attention! Some URLs passed through the config file rules and were loaded through WordPress rewrite which may slow down your website. %s Please follow this tutorial to fix the issue: %s', 'hide-my-wp' ), '<br /><br />' . join( '<br />', $mappings ) . '<br /><br />', '<a href="' . esc_url( HMWP_Classes_Tools::getOption('hmwp_plugin_website') . '/kb/theme-not-loading-correctly-website-loads-slower/' ) . '" target="_blank" class="text-warning">' . HMWP_Classes_Tools::getOption('hmwp_plugin_website') . '/kb/theme-not-loading-correctly-website-loads-slower/</a> ' . $restoreLink ), 'text-white bg-danger' );
			}
