...
class HMWP_Models_Rewrite {
	...
	public function hidePluginNames() {
		...
		try {
			...
			foreach ( $all_plugins as $plugin ) {
				...
				//If it's set to use custom plugins names mapping
				if ( HMWP_Classes_Tools::getOption( 'hmwp_hide_plugins_advanced' ) ) {
					//Check if the plugin is customized
					$plugins = (array) HMWP_Classes_Tools::getOption( 'hmw_plugins_mapping' );
					if ( in_array( $plugin, array_keys( $plugins ), true ) ) {
						if ( $plugin <> $plugins[ $plugin ] ) {
							//change with the custom plugin names
							$dbplugins['to'][]   = preg_replace( '/[^A-Za-z0-9-_]/', '', $plugins[ $plugin ] );
	...
	public function flushRewrites() {
		...
		if ( HMWP_Classes_Tools::isIIS() ) {
			...
			if ( $rewritecode <> '' ) {
				HMWP_Classes_Error::setNotification( sprintf( esc_html__( 'Flywheel detected. Add the redirects in the Flywheel Redirect rules panel %s.', 'hide-my-wp' ), '<strong><a href="' . esc_url( HMWP_Classes_Tools::getOption('hmwp_plugin_website') . '/kb/how-to-setup-hide-my-wp-on-flywheel-server/' ) . '" target="_blank" style="color: red">' . esc_html__( "Learn How To Add the Code", 'hide-my-wp' ) . '</a></strong> <br /><br /><pre>' . $rewritecode . '</pre>' . $form ), 'notice', false );
	...
	public function login_head() {
		...
		if ( HMWP_Classes_Tools::getOption( 'hmwp_remove_third_hooks' ) ) {
			...
			if ( function_exists( 'get_theme_mod' ) && function_exists( 'wp_get_attachment_image_src' ) ) {
				...
				if ( isset( $image[0] ) ) {
					echo '<style>#login h1 a, .login h1 a {background-image: url(' . esc_url( $image[0] ) . ') !important; background-position: center;}</style>';
				}
			}
	...
	function get404Page( $usetheme = false ) {
		...
		try {
			...
			echo '<html lang="' . esc_attr( get_locale() ) . '"><meta<head><title>' . esc_html__( '404 Not Found', 'hide-my-wp' ) . '</title></head><body><h1>' . esc_html__( 'Page Not Found', 'hide-my-wp' ) . '</h1><p>' . sprintf( esc_html__( 'The requested URL %s was not found on this server.', 'hide-my-wp' ), ( ( isset( $_SERVER['REQUEST_URI'] ) ) ? esc_url( $_SERVER['REQUEST_URI'] ) : '' ) ) . ' </p></body></html>';
			exit();
	...
	function get403Error() {
		...
		try {
			...
			echo '<html lang="' . esc_attr( get_locale() ) . '"><meta<head><title>' . esc_html__( '403 Forbidden', 'hide-my-wp' ) . '</title></head><body><h1>' . esc_html__( 'Forbidden', 'hide-my-wp' ) . '</h1><p>' . sprintf( esc_html__( "You don't have the permission to access %s on this server.", 'hide-my-wp' ), ( ( isset( $_SERVER['REQUEST_URI'] ) ) ? esc_url( $_SERVER['REQUEST_URI'] ) : '' ) ) . ' </p></body></html>';
			exit();
	...
	public function find_replace( $content ) {
		...
		if ( HMWP_Classes_Tools::doChangePaths() && apply_filters( 'hmwp_process_find_replace', true ) ) {
			...
			if ( HMWP_Classes_Tools::getValue( 'hmwp_debug' ) ) {
				$content .= '<pre>' . print_r( $this->_replace, true ) . '</pre>';
			}
