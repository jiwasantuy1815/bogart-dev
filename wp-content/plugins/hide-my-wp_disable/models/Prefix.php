...
class HMWP_Models_Prefix {
	...
	protected function changePrefixOptionsDatabase() {
		...
		if ( $updated_options === false ) {
			HMWP_Classes_Error::setNotification( __( 'Could not update prefix references in options table.', 'better-wp-security' ), 'error', false );
		}
	}
	...
	protected function changePrefixUserDatabase() {
		...
		foreach ( $rows as $row ) {
			...
			if ( ! $updated ) {
				HMWP_Classes_Error::setNotification( __( 'Could not update prefix references in usermeta table.', 'better-wp-security' ), 'error', false );
			}
		}
	}
