...
class HMWP_Controllers_Firewall extends HMWP_Classes_FrontController {
	...
	public function run() {
        ...
        // If firewall process is not activated, exit
			...
			if ( (int) HMWP_Classes_Tools::getOption( 'hmwp_sqlinjection_level' ) == 3 ) {
				...
				if ( isset( $_SERVER['REQUEST_URI'] ) && $_SERVER['REQUEST_URI'] <> '' ) {
					...
					if ( preg_match( '/(\^|`|<|>|\\|\|)/i', $_SERVER['REQUEST_URI'] ) ||
                        ...
                        preg_match( '/(\/)(ckfinder|fckeditor|fullclick)/i', $_SERVER['REQUEST_URI'] ) ||
			...
			if ( (int) HMWP_Classes_Tools::getOption( 'hmwp_sqlinjection_level' ) == 4 ) {
				...
				if ( isset( $_SERVER['HTTP_REFERER'] ) && $_SERVER['HTTP_REFERER'] <> '' ) {
					if ( preg_match( '/(order(\s|%20)by(\s|%20)1--)/i', $_SERVER['HTTP_REFERER'] ) ||
                        ...
                        preg_match( '/(pornhelm|pro[sz]ac|sandyauer|semalt\.com|social-buttions|todaperfeita|tramadol|troyhamby|ultram|unicauca|valium|viagra|vicodin|xanax|ypxaieo)/i', $_SERVER['HTTP_REFERER'] )
                    ) {
				...
				if ( isset( $_SERVER['HTTP_USER_AGENT'] ) && $_SERVER['HTTP_USER_AGENT'] <> '' ) {
					if ( preg_match( '/([a-z0-9]{2000,})/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(&lt;|%0a|%0d|%27|%3c|%3e|%00|0x00)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(base64_decode|bin\/bash|disconnect|eval|lwp-download|unserialize)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(acapbot|acoonbot|alexibot|asterias|attackbot|awario|backdor|becomebot|binlar|blackwidow|blekkobot|blex|blowfish|bullseye|bunnys|butterfly|careerbot|casper)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(checkpriv|cheesebot|cherrypick|chinaclaw|choppy|clshttp|cmsworld|copernic|copyrightcheck|cosmos|crescent|datacha|diavol|discobot|dittospyder)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(dotbot|dotnetdotcom|dumbot|econtext|emailcollector|emailsiphon|emailwolf|eolasbot|eventures|extract|eyenetie|feedfinder|flaming|flashget|flicky|foobot|fuck)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(g00g1e|getright|gigabot|go-ahead-got|gozilla|grabnet|grafula|harvest|heritrix|httracks?|icarus6j|jetbot|jetcar|jikespider|kmccrew|leechftp|libweb|liebaofast)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(linkscan|linkwalker|loader|lwp-download|majestic|masscan|miner|mechanize|mj12bot|morfeus|moveoverbot|netmechanic|netspider|nicerspro|nikto|ninja|nominet|nutch)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(octopus|pagegrabber|petalbot|planetwork|postrank|proximic|purebot|queryn|queryseeker|radian6|radiation|realdownload|remoteview|rogerbot|scan|scooter|seekerspid)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(semalt|siclab|sindice|sistrix|sitebot|siteexplorer|sitesnagger|skygrid|smartdownload|snoopy|sosospider|spankbot|spbot|sqlmap|stackrambler|stripper|sucker|surftbot)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(sux0r|suzukacz|suzuran|takeout|teleport|telesoft|true_robots|turingos|turnit|vampire|vikspider|voideye|webleacher|webreaper|webstripper|webvac|webviewer|webwhacker)/i', $_SERVER['HTTP_USER_AGENT'] ) ||
                        preg_match( '/(winhttp|wwwoffle|woxbot|xaldon|xxxyy|yamanalab|yioopbot|youda|zeus|zmeu|zune|zyborg|squirrly)/i', $_SERVER['HTTP_USER_AGENT'] )
                    ) {
				...
				if ( isset( $_SERVER['QUERY_STRING'] ) && $_SERVER['QUERY_STRING'] <> '' ) {
					if ( preg_match( '/^(%2d|-)[^=]+$/i', $_SERVER['QUERY_STRING'] ) ||
                        ...
                        preg_match( '/(elastix|encodeuricom|exploit|fclose|fgets|file_put_contents|fputs|fsbuff|fsockopen|gethostbyname|grablogin|hmei7|hubs_post-cta|input_file|invokefunction|(\b)load_file|open_basedir|outfile|p3dlite)/i', $_SERVER['QUERY_STRING'] ) ||
                        preg_match( '/(pass(=|%3d)shell|passthru|phpinfo|phpshells|popen|proc_open|quickbrute|remoteview|root_path|safe_mode|shell_exec|site((.){0,2})copier|sp_executesql|sux0r|trojan|udtudt|user_func_array|wget|wp_insert_user|xertive)/i', $_SERVER['QUERY_STRING'] ) ||
				...
				if ( isset( $_SERVER['REQUEST_URI'] ) && $_SERVER['REQUEST_URI'] <> '' ) {
					...
					if ( preg_match( '/(,,,)/i', $_SERVER['REQUEST_URI'] ) ||
                        ...
                        preg_match( '/(\/)((.*)header:|(.*)set-cookie:(.*)=)/i', $_SERVER['REQUEST_URI'] ) ||
                        preg_match( '/(\.(s?ftp-?)config|(s?ftp-?)config\.)/i', $_SERVER['REQUEST_URI'] ) ||
                        preg_match( '/(\/)(f?ckfinder|f?ckeditor|fullclick)/i', $_SERVER['REQUEST_URI'] ) ||
                        ...
                        preg_match( '/(\/%7e)(root|ftp|bin|nobody|named|guest|logs|sshd)\//i', $_SERVER['REQUEST_URI'] ) ||
                        preg_match( '/(\/)(\.?mad|alpha|c99|web)?sh(3|e)ll([0-9]+|\w)(\.php)/i', $_SERVER['REQUEST_URI'] ) ||
                        ...
                        preg_match( '/(\/)(htdocs|httpdocs|https?|mailman|mailto|msoffice|undefined|usage|var|vhosts|webmaster|www)(\/)/i', $_SERVER['REQUEST_URI'] ) ||
                        ...
                        preg_match( '/(\/)(logo_img|lufix|mage|marg|mass|mide|moon|mssqli|mybak|myshe|mysql|mytag_js?|nasgor|newfile|nf_?tracking|nginx|ngoi|ohayo|old-?index)(\.php)/i', $_SERVER['REQUEST_URI'] ) ||
                        ...
                        preg_match( '/(handler_to_code|hax(0|o)r|hmei7|hnap1|ibqyiove|icxbsx|indoxploi|jahat|jijle3|kcrew|keywordspy|laobiao|lock360|longdog|marijuan|mod_(aratic|ariimag))/i', $_SERVER['REQUEST_URI'] ) ||
        ...
        // If GeoIP blocking is activated
		if ( HMWP_Classes_Tools::getOption( 'hmwp_geoblock' ) ) {
			...
			if ( ! empty( $server ) ) {
				...
				foreach ( $server as $ip ) {
					...
					if ( ! empty( $blocked_countries ) ) {
						...
						/** @var HMWP_Models_Geoip_GeoLocator $database */
						$geo_locator = HMWP_Classes_ObjController::getClass( 'HMWP_Models_Geoip_GeoLocator' )->getInstance();
						$country     = $geo_locator->getCountryCode( $ip );

                        // Check if the current country is in the blocked list
						if ( $country <> '' && in_array( $country, $blocked_countries ) ) {

                            // Check the GeoBlock URLs
							$geoblock_urls = HMWP_Classes_Tools::getOption( 'hmwp_geoblock_urls' );
							if ( ! empty( $geoblock_urls ) ) {
                                // Convert to array and remove empty fields
								$geoblock_urls = json_decode( $geoblock_urls, true );
								//remove empty fields
								$geoblock_urls = array_filter( $geoblock_urls );
							}

							if ( ! empty( $geoblock_urls ) ) {

                                // Check the URI when paths are set for country blocking
								if ( isset( $_SERVER["REQUEST_URI"] ) && $_SERVER["REQUEST_URI"] <> '' ) {
									$url = untrailingslashit( strtok( $_SERVER["REQUEST_URI"], '?' ) );

									foreach ( $geoblock_urls as $path ) {
                                ...
                                // Blocked by the firewall when geoblock URLs are not set
								$this->firewallBlock( 'Geo Security' );
							}
			...
			if ( ! empty( $banlist ) ) {
				if ( isset( $_SERVER['HTTP_USER_AGENT'] ) && $_SERVER['HTTP_USER_AGENT'] <> '' ) {
					...
					foreach ( $banlist as $item ) {
						if($item <> '') {
							if ( stripos( $user_agent, $item ) !== false ) {
                                // Blocked by the firewall
								$this->firewallBlock( 'Geo Security' );
							}
						}
