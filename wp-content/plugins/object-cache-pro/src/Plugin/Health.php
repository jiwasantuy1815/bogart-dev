...
trait Health
{
    ...
    public function healthStatusTests($tests)
    {
        ...
        if ($diagnostics->clientIsPhpRedis()) {
            $tests['direct']['objectcache_phpredis_version'] = [
                'label' => 'PhpRedis version',
                'test' => function () {
                    return $this->healthTestPhpRedisVersion();
                },
            ];
        }

        if ($diagnostics->clientIsRelay()) {
            ...
            $tests['direct']['objectcache_relay_version'] = [
                'label' => 'Relay version',
                'test' => function () use ($diagnostics) {
                    return $this->healthTestRelayVersion($diagnostics);
                },
            ];
        }
        ...
        if ($diagnostics->usingRelay()) {
            ...
            $tests['direct']['objectcache_redis_tracking_keys'] = [
                'label' => 'Relay key tracking',
                'test' => function () use ($diagnostics) {
                    return $this->healthTestTrackingTableMaxKeys($diagnostics);
                },
            ];
        }
        ...
        $callback = function ($timeout = 5) {
            ...
            try {
                ...
                $content = [
                    ...
                    'url' => CLOUDWAYS_LICENSED_SITE,
                    'network_url' => CLOUDWAYS_LICENSED_SITE,
     ...
     * @return array<string, mixed>
     */
    protected function healthTestErrors()
    {
        global $wp_object_cache_errors;
    ...
    /**
     * Test whether Redis is configured to able to track all keys used by Relay.
     *
     * @param  \RedisCachePro\Diagnostics\Diagnostics  $diagnostics
     * @return array<string, mixed>
     */
    protected function healthTestTrackingTableMaxKeys(Diagnostics $diagnostics)
    {
        $keys = (int) $diagnostics->trackingTotalKeys();
        $maxKeys = (int) $diagnostics->trackingTableMaxKeys();

        if ($maxKeys > 0 && $keys > $maxKeys * 0.90) {
            return [
                'label' => 'Redis tracking at maximum capacity',
                'description' => sprintf(
                    '<p>%s</p>',
                    implode(' ', [
                        sprintf(
                            'Redis server is configured to track up to %s keys and currently %s (%s%%) keys are being tracked.',
                            number_format_i18n($maxKeys),
                            number_format_i18n($keys),
                            round($keys / $maxKeys * 100)
                        ),
                        'If the number of tracked keys exceeds the configured limit, Redis will start evicting keys and in turn force Relay to invalidate cached keys as well.',
                        'This may impact performance negatively.',
                        'It’s recommended to increase <code>tracking-table-max-keys</code> in the <code>redis.conf</code> configuration file, or ignore some groups from being cached in Relay’s in-memory cache.',
                    ])
                ),
                'badge' => ['label' => 'Object Cache Pro', 'color' => 'orange'],
                'status' => 'recommended',
                'test' => 'objectcache_redis_tracking_keys',
                'actions' => sprintf(
                    '<p><a href="%s" target="_blank">%s</a><p>',
                    'https://objectcache.pro/docs/configuration-options#relay',
                    'Learn more about ignoring groups.'
                ),
            ];
        }

        return [
            'label' => 'Redis is able to track all keys',
            'description' => sprintf(
                '<p>Redis server is tracking %s keys.</p>',
                number_format_i18n($keys)
            ),
            'badge' => ['label' => 'Object Cache Pro', 'color' => 'blue'],
            'status' => 'good',
     ...
     * preserving expiry dates and using Relay.
     *
     * @param  \RedisCachePro\Diagnostics\Diagnostics  $diagnostics
     * @return array<string, mixed>
     */
    ...
    {
        $phpredis = (string) phpversion('redis');
        $redis = (string) $diagnostics->redisVersion()->value;

        if ($diagnostics->clientIsRelay() && version_compare($redis, '6.2.7', '<')) {
            return [
                'label' => 'Relay requires 6.2.7 or newer',
                'description' => sprintf(
                    '<p>%s</p>',
                    implode(' ', [
                        "Object Cache Pro is using Relay, but the connected Redis Server ({$redis}) is too old and the object cache may go stale.",
        ...
        if ($this->config->async_flush && version_compare($redis, '4.0', '<')) {
            return [
                ...
                'description' => sprintf(
                    ...
                    implode(' ', [
                        ...
                        "but the connected Redis Server ({$redis}) is too old and does not support them.",
        ...
        if (version_compare($redis, '6.0', '<')) {
            return [
                'label' => 'Redis does not support preserving expiry dates',
                'description' => sprintf(
                    '<p>%s</p>',
                    implode(' ', [
                        'Object Cache Pro is unable to preserve the expiry of keys when de-/incrementing their value,',
                        "because the connected Redis Server ({$redis}) is too old and does not support it.",
        ...
        if ($diagnostics->clientIsPhpRedis() && version_compare($phpredis, '5.3', '<')) {
            return [
                ...
                'description' => sprintf(
                    ...
                    implode(' ', [
                        'Object Cache Pro is unable to preserve the expiry of keys when de-/incrementing their value,',
                        "because the installed PhpRedis extension ({$phpredis}) is too old and does not support it.",
    ...
    /**
     * Test whether PhpRedis is up-to-date.
     *
     * @return array<string, mixed>
     */
    protected function healthTestPhpRedisVersion()
    {
        $phpredis = (string) phpversion('redis');

        if (version_compare($phpredis, '5.3', '<')) {
            return [
                'label' => 'PhpRedis does not support preserving expiry dates',
                'description' => sprintf(
                    '<p>%s</p>',
                    implode(' ', [
                        'Object Cache Pro is unable to preserve the expiry of keys when de-/incrementing their value,',
                        "because the installed PhpRedis extension ({$phpredis}) is too old and does not support it.",
                        'Upgrade PhpRedis to version 5.3 or newer.',
                    ])
                ),
                'badge' => ['label' => 'Object Cache Pro', 'color' => 'red'],
                'status' => 'critical',
                'test' => 'objectcache_phpredis_version',
                'actions' => sprintf(
                    '<p><a href="%s" target="_blank">%s</a></p>',
                    'https://objectcache.pro/docs/phpredis',
                    'Learn more.'
        ...
        return [
            'label' => 'PhpRedis version supported',
            'description' => '<p>The PhpRedis extension version is supported.</p>',
            'badge' => ['label' => 'Object Cache Pro', 'color' => 'blue'],
            'status' => 'good',
            'test' => 'objectcache_phpredis_version',
        ];
     ...
     * @return array<string, mixed>
     */
    public function healthTestRelayConfig()
    {
        $results = [
    ...
    /**
     * Test whether Relay is up-to-date.
     *
     * @param  \RedisCachePro\Diagnostics\Diagnostics  $diagnostics
     * @return array<string, mixed>
     */
    protected function healthTestRelayVersion(Diagnostics $diagnostics)
    {
        $version = $diagnostics['versions']['relay'];

        if ($version->isError()) {
            return [
                'label' => 'Relay extension version is unsupported',
                'description' => "<p>The installed Relay extension version ({$version->value}) is not too old and not supported.</p>",
                'badge' => ['label' => 'Object Cache Pro', 'color' => 'red'],
                'status' => 'critical',
                'test' => 'objectcache_relay_version',
            ];
        }

        if ($version->isWarning()) {
            return [
                'label' => 'Relay extension version is outdated',
                'description' => "<p>The installed Relay extension version ({$version->value}) is outdated and should be upgraded.</p>",
                'badge' => ['label' => 'Object Cache Pro', 'color' => 'orange'],
                'status' => 'recommended',
                'test' => 'objectcache_relay_version',
            ];
        ...
        return [
            'label' => 'Relay version supported',
            'description' => '<p>The version of the Relay extension is supported.</p>',
            'badge' => ['label' => 'Object Cache Pro', 'color' => 'blue'],
            'status' => 'good',
            'test' => 'objectcache_relay_version',
        ];
    ...
    public function healthTestApi()
    {
        ...
        $url = static::normalizeUrl(CLOUDWAYS_LICENSED_SITE);
