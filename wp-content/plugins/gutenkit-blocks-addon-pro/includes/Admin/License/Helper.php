...
class Helper {
	...
	public function activate($key) {
		...
		if(isset($response->validate) && $response->validate == 1) {
			...
			update_option('__gutenkit_oppai__', $response->oppai);
			update_option('__gutenkit_license_key__', $response->key);
	...
	public function deactivate() {
		...
		update_option('__gutenkit_license_key__', '');
	...
	public function check_license($data = []) {
		...
		$data['oppai']       = get_option('__gutenkit_oppai__');
		$data['action']      = 'activate';
		$data['marketplace'] = \GutenkitPro::store_name();
		$data['author_name'] = \GutenkitPro::author_name();
		$data['v']           = \GutenkitPro::VERSION;

		$url = \GutenkitPro::api_url() . 'license?' . http_build_query($data);
	...
	public function get_license() {
		...
		$cached = wp_cache_get('gutenkit__license_key');
		...
		wp_cache_set('gutenkit__license_key', $license);
