...
namespace Barn2\Plugin\Document_Library_Pro\Dependencies\Lib\Service;
...
abstract class Updater implements Standard_Service, Registerable, Plugin_Activation_Listener
{
    ...
    {
        return ['version_option_name' => $this->plugin->get_slug() . '_version', 'needs_update_db_notice' => ['title' => \sprintf(__('%1$s database update required','document-library-pro' ), $this->plugin->get_name()), 'message' => \sprintf(__('<p>%1$s has been updated! To keep things running smoothly, we have to update your database to the newest version. The database update process runs in the background and may take a little while, so please be patient.</p>','document-library-pro' ), $this->plugin->get_name()), 'buttons' => ['update-db' => ['value' => 'Update Database', 'href' => '#', 'class' => 'button-primary'], 'learn-more' => ['value' => 'Learn more about updates', 'href' => 'https://barn2.com/kb/learn-more-about-updates/', 'target' => '_blank', 'class' => 'button-secondary', 'style' => 'margin-left: 8px;']]], 'update_db_complete_notice' => ['title' => \sprintf(__('%1$s database update done','document-library-pro' ), $this->plugin->get_name()), 'message' => \sprintf(__('<p>%1$s database update complete. Thank you for updating to the latest version!</p>','document-library-pro' ), $this->plugin->get_name())]];
    }
    /**
    ...
    public function show_notice()
    {
        ...
        $this->options['needs_update_db_notice']['buttons']['update-db']['href'] = $nonce_url;
    ...
    public function check_update_complete()
    {
        if ($this->is_update_complete()) {
            $admin_notice = new Notices();
            $admin_notice->add($this->plugin->get_slug() . '_update_db_complete_notice', $this->options['update_db_complete_notice']['title'], $this->options['update_db_complete_notice']['message'], ['type' => 'success', 'capability' => 'install_plugins', 'additional_classes' => ['barn2-update-complete-notice']]);
            $admin_notice->boot();
        }
    }
    /**
    ...
    {
        if (!\current_user_can('manage_options')) {
            \wp_die(__('You do not have sufficient permissions to perform this action.','document-library-pro' ));
        }
        // If already locked, stop.
        if ($this->is_upgrade_locked()) {
            \wp_die(__('An upgrade is already in progress. Please wait until it completes.','document-library-pro' ));
        }
