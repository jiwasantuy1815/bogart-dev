<?php

/**
 * @package   Barn2\barn2-settings-api
 * @author    Barn2 Plugins <support@barn2.com>
 * @license   GPL-3.0
 * @copyright Barn2 Media Ltd
 */
namespace Barn2\Plugin\Document_Library_Pro\Dependencies\Barn2\Settings_API\Fields;

use Barn2\Plugin\Document_Library_Pro\Dependencies\Barn2\Settings_API\Traits\With_Button_Data;
/**
 * Button field class.
 * Represents a button field.
 */
class Button extends Field
{
    /** {@inheritDoc} */
    protected $type = 'button';
    use With_Button_Data;
    #[\ReturnTypeWillChange]
    public function jsonSerialize()
    {
        $field = parent::jsonSerialize();
        $field['buttonData'] = $this->get_button_data();
        return $field;
    }
}
