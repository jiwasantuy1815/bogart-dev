...
final class Util {
	...
	public static function count_expired_posts() {
		...
		$cache_key = 'dlp_expired_posts_count';

		$counts = wp_cache_get( $cache_key, 'counts' );

		$counts = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) AS total_expired_posts
				FROM {$wpdb->posts} p
				JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id
				WHERE pm.meta_key = %s
				AND pm.meta_value < %d",
				'_dlp_expiry_timestamp',
				time()
			)
		);
	 ...
	 * @return array
	 */
	public static function get_eligible_auto_thumbnail_documents( $batch_size ) {
		$file_type_terms = [ 'jpg', 'jpeg', 'gif', 'png' ];

		if ( Media::has_pdf_previews() ) {
			$file_type_terms[] = 'pdf';
		}
		...
		$args = [
			...
			'meta_query'             => [
				...
				[
					'key'     => '_dlp_auto_thumbnail_processed',
					'compare' => 'NOT EXISTS',
				],
				[
					'key'     => '_dlp_document_link_type',
					'value'   => 'file',
					'compare' => '=',
				],
			],
			'tax_query'              => [
				[
					'taxonomy' => Taxonomies::FILE_TYPE_SLUG,
					'field'    => 'slug',
					'terms'    => $file_type_terms,
				],
			],
			'fields'                 => 'ids',
		];
