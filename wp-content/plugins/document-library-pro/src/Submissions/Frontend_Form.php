 ...
 */
class Frontend_Form implements Registerable, Premium_Service, JsonSerializable {

	const POST_TYPE = 'dlp_document';
	...
	public function init_fields() {
		...
		$custom_taxonomies = $this->get_custom_taxonomies( $priority = 11 );
		...
		$cf_fields = $this->get_custom_fields( $taxonomy_counts = count( $custom_taxonomies ) );
		if ( ! empty( $cf_fields ) ) {
			foreach ( $cf_fields as $key => $cf_field ) {
				$fields[ $key ] = $cf_field;
			}
		}
	...
	/**
	 * Retrieves custom taxonomies for the defined post type, excluding defaults.
	 *
	 * @param int $priority Starting priority for the taxonomies. Default is 11.
	 *
	 * @return array Associative array of custom taxonomies with details like label, type,
	 *               taxonomy name, placeholder, required status, and priority.
	 */
	public function get_custom_taxonomies( $priority = 11 ) {
		$custom_taxonomies = [];
		...
		$default_taxonomies = [
			...
			Taxonomies::FILE_TYPE_SLUG,
			Taxonomies::DOCUMENT_DOWNLOAD_SLUG,
		];

		$all_taxonomies = get_object_taxonomies( self::POST_TYPE, 'objects' );

		foreach ( $all_taxonomies as $key => $taxonomy ) {
			if ( in_array( $key, $default_taxonomies ) ) {
				continue;
			}
			...
			$custom_taxonomies[ $taxonomy->name ] = [
				...
				'placeholder' => sprintf( __( 'Select one or more %s...', 'document-library-pro' ), strtolower( $taxonomy->label ) ),
				'required'    => false,
				'priority'    => $priority,
			];
	...
	/**
	 * Gets the list of custom fields for the post type
	 *
	 * @return array
	 */
	public function get_custom_fields( $taxonomy_counts ) {
		$custom_fields = [];
		$acf_priority  = 12 + $taxonomy_counts;

		// Get ACF custom fields.
		if ( Util::is_acf_active() ) {
			$custom_fields = Util::get_acf_custom_fields( self::POST_TYPE, $acf_priority );
		}

		$ept_priority = $acf_priority + count( $custom_fields );
		// Get Easy Post Types custom fields.
		if ( Util::is_ept_active() ) {
			$custom_fields = array_merge( $custom_fields, Util::get_ept_custom_fields( self::POST_TYPE, $ept_priority ) );
		}
	...
	protected function get_posted_fields() {
		...
		foreach ( $this->fields as $key => $field ) {
			...
			} elseif ( str_starts_with( $key, 'ept:' ) ) {
				$is_ept_field = true;
			...
			if ( $handler ) {
				...
				$value = call_user_func( [ $this, "get_posted_{$field_type}_field" ], $key, $field );
			} else {
				$value = $this->get_posted_field( $key, $field );
			}

			if ( $is_acf_field ) {
				$values['acf'][ $key ]                  = $value;
				$this->fields[ 'acf:' . $key ]['value'] = $value;
			} elseif ( $is_ept_field ) {
				$values['ept'][ $key ]                  = $value;
				$this->fields[ 'ept:' . $key ]['value'] = $value;
			} else {
				$values[ $key ]                = $value;
	...
	protected function validate_fields( $values ) {
		foreach ( $this->fields as $key => $field ) {
			if (
				...
				return new WP_Error( 'validation-error', sprintf( __( '%s is a required field', 'document-library-pro' ), wp_kses_post( $field['label'] ) ) );
			}
			if ( ! empty( $field['taxonomy'] ) && in_array( $field['type'], [ 'taxonomy' ], true ) ) {
				...
				foreach ( $check_value as $term ) {
					if ( ! term_exists( $term, $field['taxonomy'] ) ) {
						...
						return new WP_Error( 'validation-error', sprintf( __( '%s is invalid', 'document-library-pro' ), wp_kses_post( $field['label'] ) ) );
					}
				}
			}
			if ( 'file' === $field['type'] ) {
					...
					foreach ( $check_value as $file_url ) {
						if ( ! is_numeric( $file_url ) ) {
							...
							if ( $reject_external_files && 0 !== strpos( $file_url, $baseurl ) ) {
								throw new Exception( esc_html__( 'Invalid image path.', 'document-library-pro' ) );
							}
						}
						// Check mime types.
						if ( ! empty( $field['allowed_mime_types'] ) ) {
							...
							if ( ! is_numeric( $file_url ) && $file_info && ! in_array( $file_info['type'], $field['allowed_mime_types'], true ) ) {
								...
								throw new Exception( wp_kses_post( sprintf( __( '"%1$s" (filetype %2$s) needs to be one of the following file types: %3$s', 'document-library-pro' ), $field['label'], $file_info['ext'], implode( ', ', array_keys( $field['allowed_mime_types'] ) ) ) ) );
							}
						}
						...
						if ( empty( $file_url ) ) {
							throw new Exception( esc_html__( 'Invalid attachment provided.', 'document-library-pro' ) );
						}
					}
				}
			}
	...
	public function process_form() {
		try {
			...
			$args = [
				...
				'acf'          => isset( $values['acf'] ) ? $values['acf'] : [],
				'ept'          => isset( $values['ept'] ) ? $values['ept'] : [],
			];

			// Include custom taxonomies data to the document args
			foreach ( $custom_taxonomies as $key => $taxonomy ) {
				if ( ! empty( $values[ $key ] ) ) {
					$args['taxonomies'][ $key ] = $values[ $key ];
				}
			}
