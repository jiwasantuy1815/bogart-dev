...
class Folder_Tree {
	...
	private function init_category_options() {
		...
		$this->include_terms = array_filter(
			array_map(
				function ( $term ) {
		...
		$this->exclude_terms = array_filter(
			array_map(
				function ( $term ) {
					$term_split = explode( ':', $term, 2 );
	...
	private function generate_category_folder_html( $categories, $root = false, $shortcode_atts = [] ) {
		...
		<div class="dlp-folders-container" style="display:none;">
			...
			if ( isset( Options::get_settings()['folder_design'] ) && Options::get_settings()['folder_design'] === 'default' ) {
				$top_folder_color = '#f6b900';
				$sub_folder_color = '#f6b900';
			} else {
				$settings = Options::get_settings();
				$top_folder_color = isset($settings['folder_icon_color']) ? $settings['folder_icon_color'] : '#f6b900';
				$sub_folder_color = isset($settings['folder_icon_subcolor']) ? $settings['folder_icon_subcolor'] : '#f6b900';
			}
			?>
				...
				foreach ( $categories as $category_id => $subcategories ) :
					...
					<li class="dlp-folder dlp-folder <?php echo esc_attr( $status ); ?>" data-category-id="<?php echo esc_attr( $category_id ); ?>">
						<span class="dlp-category" tabindex="0" role="button" aria-expanded="false">
							...
							$folder_color = $is_top_level ? $top_folder_color : $sub_folder_color;
							SVG_Icon::render( 'folder', [], $folder_color );
							SVG_Icon::render( 'folder_open', [], $folder_color );
							?>
							...
							if ( $description !== '' && apply_filters( 'document_library_pro_should_display_category_description_in_folder', false ) ) {
								if ( isset( Options::get_user_shortcode_options()['shortcodes'] ) && Options::get_user_shortcode_options()['shortcodes'] ) {
