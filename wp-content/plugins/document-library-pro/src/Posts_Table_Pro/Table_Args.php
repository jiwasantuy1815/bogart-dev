...
class Table_Args {
	...
	public $grid_columns;
	public $grid_links;
	public $grid_document_title_link;
	public $grid_filename_link;
	public $table_document_title_link;
	public $table_filename_link;
	public $new_tab_links;

	public $hide_header;
	public $sticky_header;
	public $accent_neutralise;
	...
	private function parse_args( array $args ) {
		...
		$validation = apply_filters(
			...
			[
				...
				'sticky_header'             => FILTER_VALIDATE_BOOLEAN,
				'accent_neutralise'         => FILTER_VALIDATE_BOOLEAN,
				'image_size'                => [
					'filter'  => FILTER_CALLBACK,
		...
		foreach ( [ 'widths', 'priorities', 'column_breakpoints', 'column_type', 'status', 'exclude', 'include', 'exclude_category' ] as $arg ) {
		...
		if ( in_array( $this->sort_order, [ 'numasc', 'numdesc' ] ) ) {
			$this->num_order  = $this->sort_order === 'numasc' ? 'asc' : 'desc';
			$this->sort_order = $this->sort_order === 'numasc' ? 'asc' : 'desc';
		}

		// Check for numeric ordering
		$col_type        = Columns_Util::get_column_type( $this->column_type, $this->sort_by );
		$this->num_order = $col_type === 'number' ? true : false;

		if ( ! in_array( $this->sort_order, [ 'asc', 'desc' ], true ) ) {
			// Default to descending order for date sorting, ascending for everything else.
			$this->sort_order = in_array( $this->sort_by, array_merge( [ 'date', 'date_modified' ], $this->column_type ), true ) ? 'desc' : 'asc';
		}
