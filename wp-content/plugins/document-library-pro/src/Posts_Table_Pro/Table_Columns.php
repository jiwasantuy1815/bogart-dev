...
class Table_Columns {
	...
	public function get_column_heading( $index, $column ) {
		...
		return apply_filters( 'document_library_pro_column_heading_' . $unprefixed_col, $heading );
	}
	...
	public function get_column_heading_class( $index, $column ) {
		...
		return implode( ' ', apply_filters( 'document_library_pro_header_class_' . $unprefixed_col, $class ) );
	}
	...
	public function get_column_priority( $index, $column ) {
		...
		$priority = apply_filters( 'document_library_pro_column_priority_' . $unprefixed_col, $priority );
	...
	public function get_column_width( $index, $column ) {
		$unprefixed_col = Columns_Util::unprefix_column( $column );
		$width = apply_filters( 'document_library_pro_column_width_' . $unprefixed_col, '' );
	...
	public function is_click_filterable( $column ) {
		...
		$click_filterable = apply_filters( 'document_library_pro_column_click_filterable_' . $unprefixed_col, $click_filterable );
	...
	public function is_searchable( $column ) {
		...
		if ( $searchable ) {
			$searchable = apply_filters( 'document_library_pro_column_searchable', $searchable, $unprefixed_col );
			$searchable = apply_filters( 'document_library_pro_column_searchable_' . $unprefixed_col, $searchable );
		}
	...
	public function is_sortable( $column ) {
		...
		if ( $sortable ) {
			$sortable = apply_filters( 'document_library_pro_column_sortable', $sortable, $unprefixed_col );
			$sortable = apply_filters( 'document_library_pro_column_sortable_' . $unprefixed_col, $sortable );
		}
