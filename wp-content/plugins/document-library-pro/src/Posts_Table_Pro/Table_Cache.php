...
class Table_Cache {
	...
	public function update_data( $data ) {
		if ( $this->data_caching_enabled() ) {
			/**
			 * Filters the maximum number of posts that can be cached.
			 *
			 * @param int $max_cache_size The maximum number of posts that can be cached.
			 * @param Table_Cache $table_cache The table cache instance.
			 * @return int The maximum number of posts that can be cached.
			 */
			$max_cache_size = apply_filters( 'document_library_pro_max_cache_size', 1000, $this );

			if ( count( $data ) <= $max_cache_size ) {
