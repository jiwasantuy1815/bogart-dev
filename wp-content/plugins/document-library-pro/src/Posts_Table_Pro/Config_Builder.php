...
class Config_Builder {
	...
	public function get_config() {
		...
		foreach ( $this->columns->get_columns() as $column ) {
			...
			// Set column type to 'num' when num_order presents
			$col_type = Columns_Util::get_column_type( $this->args->column_type, $column );
			if ( $col_type === 'number' ) {
				$column_type = 'num';
			}
