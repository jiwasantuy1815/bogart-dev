...
class Posts_Table {
	...
	private function add_attributes() {
		...
		if ( $this->args->hide_header ) {
			$table_class[] = 'no-header';
		}

		if ( $this->args->sticky_header ) {
			$table_class[] = 'sticky-header';
		}

		$theme_settings = Options::get_setting_table_styling();
		if ( ! empty( $theme_settings['corner_style'] ) ) {
			$table_class[] = $theme_settings['corner_style'];
		}
