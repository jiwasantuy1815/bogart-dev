...
class Table_Query {
	...
	private function build_base_posts_query() {
		...
		$query_args = $this->add_ordering_args( $query_args );

		/**
		 * Filter the base query arguments before ordering is applied.
		 *
		 * @param array       $query_args The WP_Query arguments.
		 * @param Table_Query $this       The Table_Query instance.
		 * @return array The filtered query arguments.
		 */
		return apply_filters( 'document_library_pro_base_query_args', $query_args, $this );
	}
	...
	private function build_meta_query() {
			...
			foreach ( $field_array as $field ) {
				...
				if ( count( $field_split ) === 2 ) {
					...
					if ( Util::is_acf_active() ) {
						$compare     = 'REGEXP';
						$field_value = sprintf( '(^%1$s$|s:%2$u:"%1$s";|i:\d+;s:%2$u:"%1$s";)', preg_quote( $field_value, '/' ), strlen( $field_value ) );
					}
		...
		if ( Columns_Util::is_custom_field( $this->args->sort_by ) ) {
			...
			$column_type = Columns_Util::get_column_type( $this->args->column_type, $this->args->sort_by );
			...
			$is_retreiving_specific_cf_field_posts = Columns_Util::is_retreiving_specific_cf_field_posts( $field, $this->args->cf );

			$sort_query_clauses = [
				'relation'                 => 'OR',
				'document_library_pro_order_clause' =>
				[
					'key'  => $field,
					'type' => apply_filters( 'wc_posts_table_sort_by_custom_field_type', $type, $field ),
				],
			];

			if ( ! $is_retreiving_specific_cf_field_posts ) {
				$sort_query_clauses['document_library_pro_order_clause2'] = [
					'key'     => $field,
					'compare' => 'NOT EXISTS',
				];
			}

			// Merge the existing meta query with the sorting meta query
			if ( ! empty( $meta_query ) ) {
				$meta_query = [
					'relation' => 'AND',
					$meta_query,
				...
				$meta_query = $sort_query_clauses;
			}
		}

		/**
		 * Filter the meta query before it is used in WP_Query.
		 *
		 * @param array       $meta_query The meta query array containing all meta query clauses
		 * @param Table_Query $this       The Table_Query instance
		 * @return array The filtered meta query
		 */
	 ...
	 *
	 * If sorting by a custom field (meta query), sets up orderby clauses for the meta value
	 * and handles posts with/without the meta field. Otherwise maps the sort_by parameter
	 * to the appropriate WP_Query orderby value.
	 *
	 * @param array $query_args The existing query arguments
	 * @return array The modified query arguments with ordering parameters added
	 */
	private function add_ordering_args( $query_args ) {
		$order   = strtoupper( $this->args->sort_order );
		$orderby = $this->args->sort_by;

		if ( ! empty( $query_args['meta_query']['document_library_pro_order_clause'] ) && ! empty( $query_args['meta_query']['document_library_pro_order_clause2'] ) ) {
			$query_args['orderby']['meta_value_num']              = $order;
			$query_args['orderby']['product_table_order_clause']  = $order;
			$query_args['orderby']['product_table_order_clause2'] = $order;
	...
	public function filter_wp_posts_selected_columns( $fields, $query ) {
		...
		if ( array_diff( [ 'content', 'excerpt' ], $this->args->columns ) ) {
			...
			// Check if 'content' and 'excerpt' are in the combined columns
			$content_field_needed = false;
			$excerpt_field_needed = false;

			foreach ( $this->args->columns as $column ) {
				if ( Columns_Util::is_combined_column( $column ) ) {
					foreach ( explode( ';', $column ) as $sub_column ) {
						if ( $sub_column === 'content' ) {
							$content_field_needed = true;
						}
						if ( $sub_column === 'excerpt' ) {
							$excerpt_field_needed = true;
						}
					}
				}
			}
			...
			if ( in_array( 'content', $this->args->columns, true ) || $content_field_needed ) {
				$posts_columns[] = 'post_content';
			}

			// Only select post_excerpt if it's definitely needed
			if ( in_array( 'excerpt', $this->args->columns, true ) || $excerpt_field_needed ) {
