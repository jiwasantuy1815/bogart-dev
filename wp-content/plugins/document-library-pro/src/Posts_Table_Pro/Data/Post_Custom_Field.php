...
class Post_Custom_Field extends Abstract_Table_Data {
	...
	public function __construct( $post, $field, $links = '', $image_size = '', $date_format = '', $column_type = [] ) {
		parent::__construct( $post, $links );

		$this->field            = $field;
		$this->image_size       = $image_size ?: 'thumbnail';
		$this->date_format      = $date_format;
		$this->column_type     = (array) $column_type;
	...
	/**
	 * Check if this custom field is configured as a date field.
	 *
	 * @return bool Whether this is a date field
	 */
	private function is_date_field() {
		foreach ( $this->column_type as $column ) {
			// Check for entries that contain this field name and end with ::date
			if ( preg_match( '/' . preg_quote( $this->field, '/' ) . '::date$/', $column ) ) {
				return true;
			}
		}
		// Maintain backward compatibility with the old approach
		return in_array( 'date', $this->column_type, true );
	}
