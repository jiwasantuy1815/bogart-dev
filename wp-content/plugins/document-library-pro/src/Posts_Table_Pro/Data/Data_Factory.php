	...
	public function create( $column, WP_Post $post ) {
		...
		switch ( $column ) {
			...
			case 'button':
				...
				break;
			default:
				if ( $combined_columns = Columns_Util::get_combined_column( $column ) ) {
					$data_obj = new Post_Combined_Column( $post, $combined_columns, $this->args );
				} elseif ( $taxonomy = Columns_Util::get_custom_taxonomy( $column ) ) {
					$data_obj = new Post_Custom_Taxonomy( $post, $taxonomy, $this->args->links, $this->args->date_format, $this->args->column_type );
				} elseif ( $field = Columns_Util::get_custom_field( $column ) ) {
					$data_obj = new Post_Custom_Field( $post, $field, $this->args->links, $this->args->image_size, $this->args->date_format, $this->args->column_type );
		...
		/**
		 * Filter the data object for the given column.
		 *
		 * @param Table_Data_Interface $data_obj The data object.
		 * @param string $column The column to create the data object for.
		 * @param WP_Post $post The post to retrieve data from.
		 * @param Table_Args $args The table args object.
		 */
		return apply_filters( 'document_library_pro_table_data_object', $data_obj, $column, $post, $this->args );
	}
