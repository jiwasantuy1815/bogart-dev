...
class CSV_Importer {
	...
	public function __construct( $file, $params = [] ) {
		...
		add_filter( 'document_library_pro_import_document_url', [ 'Barn2\Plugin\Document_Library_Pro\Util\Media', 'maybe_sanitize_dropbox_link' ] );
		add_filter( 'document_library_pro_import_document_url', [ 'Barn2\Plugin\Document_Library_Pro\Util\Media', 'maybe_get_google_drive_download_link' ] );
		add_filter( 'document_library_pro_import_file_array', [ 'Barn2\Plugin\Document_Library_Pro\Util\Media', 'maybe_update_imported_google_drive_file_name' ], 10, 2 );
	 ...
	 * @return string|null
	 */
	public function parse_expiry_field( $value ) {
		if ( empty( $value ) ) {
			return null;
		}

		// validate date YYYY-MM-DD HH:MM (with the time optional).
		if ( ! preg_match( '/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])([ ]([01][0-9]|2[0-3]):[0-5][0-9])?$/', $value ) ) {
			return null;
		}
	...
	protected function get_formatting_callback() {
		...
		$data_formatting = [
			...
			'expiry_timestamp' => [ $this, 'parse_expiry_field' ],
		];
