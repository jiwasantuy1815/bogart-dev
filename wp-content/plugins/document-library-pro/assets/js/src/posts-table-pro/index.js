( function( $, window, params ) {
	...
	/**
	 * Table border corner styles
	 * This approach is to make the table border corner styles work appropritately when fully-rounded or rounded-corners are selected. 
	 * 
	 * @since 3.3.0
	 */
	function roundTableCorners( table ) {
		if ( table.$table.hasClass('fully-rounded') || table.$table.hasClass('rounded-corners') ) {
			// Select all th elements of header
			let tableHeaderCols = table.$table.find('thead tr').find('th').not('.dtr-hidden');
			if ( tableHeaderCols.length > 0 ) {
				tableHeaderCols.css('border-radius', '');
				tableHeaderCols.last().css('border-radius', '0 16px 0 0');
			}

			// Select the last td element of the last row
			let lastTableRowCols = table.$table.find('tbody tr:last-child').find('td').not('.dtr-hidden');
			if ( lastTableRowCols.length > 0 ) {
				lastTableRowCols.css('border-radius', '');
				lastTableRowCols.last().css('border-radius', '0 0 16px 0');
			}

			// Select all th elements of header
			let tableFooterCols = table.$table.find('tfoot tr').find('th').not('.dtr-hidden');
			if ( tableFooterCols.length > 0 ) {
				tableFooterCols.css('border-radius', '');
				tableFooterCols.last().css('border-radius', '0 0 16px 0');
			}
		}
	}
	...
	PostsTable.prototype.initResetButton = function() {
		let table = this,
			resetLinkVisibility = table.config.resetButton ? '' : 'style="display: none;"' ;

		let $resetButton =
			$( '<div class="posts-table-reset"><a class="reset" href="#" ' + resetLinkVisibility + '>' + params.language.resetButton + '</a></div>' )
	...
	PostsTable.prototype.initStickyHeader = function() {
		const table = this,
		$floatTable = table.$table;

		if ( ! table.config.stickyHeader ) {
			return table;
		}

		$floatTable.floatThead({
			top: function( $table ) {
				return table.config.scrollOffset;
			},
		});
	...
	function onReset( event ) {
		...
		// Resize and recalculate columns for responsive breakpoints
		table.$table.trigger('responsive-resize.dt', { forceResize: true });
	}
	...
	function onSearch( event, settings ) {
		...
		// Check if there are no matching rows after search
		let info = table.getDataTable().page.info();
		let resultsWereEmpty = info.recordsDisplay === 0;

		if (resultsWereEmpty) {
			table.$table.addClass('datatables-no-rows-found');
		} else {
			table.$table.removeClass('datatables-no-rows-found');
			if (window.wasEmpty) {
				table.$table.trigger('responsive-resize.dt', { forceResize: true });
			}
		}
    	...
    	window.wasEmpty = resultsWereEmpty;
	...
	function onResponsiveResize( event, data ) {
		let table = event.data.table;
		const forceResize = data?.forceResize;
		
		if ( ! table.$table.is( ':visible' ) || forceResize ) {
		...
		// Make table border corner style
		roundTableCorners( table );
	}
