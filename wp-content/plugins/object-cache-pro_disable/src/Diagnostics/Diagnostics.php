...
class Diagnostics implements ArrayAccess
{
    ...
    protected function gatherRedisStatistics()
    {
        ...
        $this->data[self::STATISTICS]['redis-tracking'] = $this->connection
            ? $this->keyTracking()
            : null;
    }
    ...
    {
        if (! $this->config) {
            return new WP_Error('hello', 'Missing configuration.');
        }

        try {
            $this->config->connector::connect($this->config)->ping();
        } catch (Throwable $exception) {
            return new WP_Error('hello', $exception->getMessage());
        }
    ...
    public function serverType()
    {
        ...
        if ($info = $this->redisInfo()) {
            if (isset($info['dragonfly_version'])) {
                $type = 'DragonflyDB';
            } elseif (isset($info['keydb_version'])) {
                $type = 'KeyDB';
            } elseif (isset($info['valkey_version'])) {
                $type = 'Valkey';
            } else {
                $type = 'Redis';
            }
    ...
    protected function redisConfig()
    {
        ...
        if (! $config) {
            $config = $this->connection->command('config', ['GET', '*']);
        }
