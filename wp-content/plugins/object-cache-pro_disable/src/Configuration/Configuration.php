...
final class Configuration
{
    ...
    /**
     * The time-to-live for keys in the `*-queries` cache groups.
     *
     * @var int
     */
    protected $queryttl = 60 * 60 * 24;
    ...
    public function init()
    {
        ...
        $this->relay->adaptive = (object) $this->relay->adaptive;
    ...
    /**
     * Set the time-to-live for keys in the `*-queries` cache groups.
     *
     * @param  int|string  $seconds
     * @return void
     */
    public function setQueryTTL($seconds)
    {
        if (\is_string($seconds) && \filter_var($seconds, FILTER_VALIDATE_INT) !== false) {
            $seconds = (int) $seconds;
        }
        ...
        if (! \is_int($seconds)) {
            throw new ConfigurationInvalidException(
                \sprintf('`queryttl` must be an integer, %s given', \gettype($seconds))
            );
        }

        if ($seconds < 0) {
            throw new ConfigurationInvalidException('`queryttl` must be `0` (forever) or a positive integer (seconds)');
        }

        $this->queryttl = $seconds;
    }
    ...
    public function setAnalytics($analytics)
    {
        ...
        foreach ($config as $option => &$value) {
            ...
            switch ($option) {
                case 'retention':
                    $value = (int) $analytics[$option];
                    break;
                case 'sample_rate':
                    $value = (float) $analytics[$option];
                    break;
                case 'include':
                    if (! \is_array($analytics[$option])) {
                        $analytics[$option] = [$analytics[$option]];
                    }
                    $value = array_unique(array_merge($value, array_filter($analytics[$option], 'is_string')));
                    break;
                default:
                    $value = (bool) $analytics[$option];
                    break;
            }
        }

        $this->analytics = (object) $config;
    }
    ...
    public function setRelay($relay)
    {
        ...
        foreach (\array_keys((array) $this->relay) as $option) {
            ...
            if ($option === 'adaptive') {
                $this->setRelayAdaptive($relay['adaptive']);
                continue;
            }
    ...
    /**
     * Set the Relay adaptive cache configuration.
     *
     * @param  array<string, mixed>  $options
     * @return void
     */
    protected function setRelayAdaptive($options)
    {
        if (! \is_array($options)) {
            throw new ConfigurationInvalidException(
                \sprintf('`relay.adaptive` must be an array, %s given', \gettype($options))
            );
        }

        $invalid = \array_diff_key($options, (array) $this->relay->adaptive);

        if (! empty($invalid)) {
            throw new ConfigurationInvalidException(
                \sprintf('`relay.adaptive.%s` is not a valid configuration option', key(array_slice($invalid, 0, 1)))
            );
        }

        foreach (\array_keys((array) $this->relay->adaptive) as $option) {
            if (! \array_key_exists($option, $options)) {
                continue;
            }

            if ($option === 'formula') {
                $formula = $options[$option];

                if (! \in_array($formula, ['scaled', 'pure'])) {
                    throw new ConfigurationInvalidException("Relay formula `{$formula}` is not supported");
                }

                $this->relay->adaptive->formula = $formula;
                continue;
            }

            if ($option === 'events') {
                $events = $options[$option];

                if (! \is_int($events) || $events < 0) {
                    throw new ConfigurationInvalidException('`relay.adaptive.events` must an integer');
                }

                $this->relay->adaptive->events = $events;
                continue;
            }

            if ($option === 'ratio') {
                $ratio = $options[$option];

                if (! \is_float($ratio) || $ratio < 0) {
                    throw new ConfigurationInvalidException('`relay.adaptive.ratio` must a float');
                }

                $this->relay->adaptive->ratio = $ratio;
                continue;
            }

            if ($option === 'depth') {
                $depth = $options[$option];

                if (! \is_int($depth) || $depth < 1 || $depth > 8) {
                    throw new ConfigurationInvalidException('`relay.adaptive.depth` must a integer between 1 and 8');
                }

                $this->relay->adaptive->depth = $depth;
                continue;
            }

            if ($option === 'width') {
                $width = $options[$option];

                if (! \is_int($width) || $width < 512 || $width > PHP_INT_MAX) {
                    throw new ConfigurationInvalidException('`relay.adaptive.depth` must a integer between 512 and 2^31');
                }

                $this->relay->adaptive->width = $width;
                continue;
            }
        }
    }
    ...
    public function diagnostics()
    {
        ...
        $formatter = static function ($name, $value) use ($encodeJson) {
            ...
            if (in_array($name, ['maxttl', 'queryttl', 'timeout', 'read_timeout']) && ! is_null($value)) {
                $value = $value + 0;
