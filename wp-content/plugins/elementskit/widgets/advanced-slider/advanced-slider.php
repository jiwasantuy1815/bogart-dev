...
class ElementsKit_Widget_Advanced_Slider extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'widgetarea_edit_button_position_hr',
			[
		...
		$this->add_control(
			'widgetarea_edit_button_position',
			[
				'label' => esc_html__('Edit Button Position', 'elementskit'),
				'description' => esc_html__('Adjusts the Edit Content button position if it gets covered by fixed elements. This only affects the editor view and has no impact on the frontend.', 'elementskit'),
				...
				'default' => 'top-right',
				'selectors' => [
					'{{WRAPPER}} .widgetarea_warper .widgetarea_warper_edit' => '{{VALUE}}',
				],
			]
		);
	...
	protected function render_raw( ) {
        ...
        $advanced_slider_options = [
            ...
            'sliderOptions' => [     
                ...
                'autoPlayDelay' => isset($ekit_slider_auto_play_delay) ? $ekit_slider_auto_play_delay : 3000,
				'breakpointsOption' => [
					360 => [
					...
					<?php foreach ($ekit_slider_tab_items as $i => $tab) : ?>
						<div class="swiper-slide ekit-swiper-slide elementor-repeater-item-<?php echo esc_attr( $tab[ '_id' ] ); ?>">
							<?php 
								...
								echo Widget_Area_Utils::parse( $tab['ekit_slider_tab_content'], $this->get_id(), $tab[ '_id' ], $ekit_ajax_template, ($i + 1) ); 
